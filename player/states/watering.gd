...
func enter(_previous_state: String, data: Dictionary = {}) -> void:
	...
	player.animation_tree["parameters/bucket/blend_position"] = player.facing_dir * Vector2(1, -1)
	player.animation_state.travel("bucket")
	previous_state = _previous_state


func update(_delta: float) -> void:
	if !player.animation_state.get_current_node() == "bucket":
